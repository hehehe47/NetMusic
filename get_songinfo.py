        ...
        if length != 0:
            while i < length:
                ...
                if flag == 1:
                    ...
                    MongoDB.insert_recent_list_show(user_id, singer_name, singer_id, song_name, song_id, album_name,
                                               album_id, score)  # 数据库循环问题
                else:
                    # MongoDB.insert_all_list(user_id, singer_name, singer_id, song_name, song_id, album_name, album_id,
                    #                         score)
                    MongoDB.insert_all_list_show(user_id, singer_name, singer_id, song_name, song_id, album_name, album_id,
                                            score)
                i = i + 1
                #############################   MONGODB(存数据一定要设条件recent和all   #########################
